...
def segment_level_scoring(samples: Dict[str, List[str]], metric: str,lp):
    ...
    if metric == "chrF":
        ...
        #scores = np.random.random(size = len(samples["ref"]))
        #scores = am.getModuleScore(samples["mt"], samples["ref"],lp)
        
        scores = m.getChunkBasedScore(samples["mt"], samples["ref"])
...
if __name__ == "__main__":
    ...
    segment_data.to_csv("scores/{}.seg.score_chunk".format(args.baseline), index=False, header=False, sep="\t")
